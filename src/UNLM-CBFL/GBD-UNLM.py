...
if __name__ == "__main__":
    ...
    beta = 0.7   # beta is the dissimilarity level     
    ...
    def Primal_SP(x_sol):
        ...
        Phi = b*(np.sum(exp_Ubeta*y_sol,axis=1))**beta/((np.sum(exp_Ubeta*y_sol,axis=1))**beta + exp_U_0)
        der_R = np.zeros((I,J))
        for i in range(I):
            der_R[i] = b[i] * exp_Ubeta[i] * exp_U_0[i] * beta * (exp_Ubeta[i]@y_sol[i])**(beta-1) / ((exp_Ubeta[i]@y_sol[i])**beta + exp_U_0[i])**2
    ...
    print("Profit check",round(np.sum(Phi),1))
